...
main() {
  ...
  build_ios 'CTSimplePing' 'build/CTSimplePing' 'CTSimplePing'
...
build_ios() {
  ...
  checksum=`swift package compute-checksum Release/CTSimplePing.xcframework.zip`
  awk -v value="\"$checksum\"" '!x{x=sub(/checksum:.*/, "checksum: "value)}1' $package_file > $package_tmp_file \
      && mv $package_tmp_file $package_file
  
  version=`cat version.txt`
  p_framework="CTSimplePing.xcframework.zip"
  github_url="https://github.com/CleverTap/CTSimplePing/releases/download"
